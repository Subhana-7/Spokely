...
export class ConnectionService {
  ...
  async sendConnectionRequest(senderId: string, referralCode: string) {
    ...
    if (receiver._id.equals(senderId)) throw new Error("You can't connect with yourself");

    const existing = await this.repo.findByReferralCode(new Types.ObjectId(senderId), receiver._id);
    if (existing) throw new Error('Connection already exists');

    return await this.repo.createConnection(new Types.ObjectId(senderId), receiver._id);
  }
