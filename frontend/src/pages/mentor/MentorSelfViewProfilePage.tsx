...
const MentorProfile = () => {
  ...
  const handleSavePlans = () => {
    ...
    saveMentorPlans(user.id, plans)
      ...
      .catch(() =>
        setShowSaveModal({ show: true, message: "Failed to save plans." })
      );
  };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
          <div className="xl:col-span-1">
            <SpokelyCard className="p-8 mb-6 bg-white/10 backdrop-blur-lg border border-white/20">
              <div className="text-center">
                ...
                <Badge
                  variant="mentor"
                  className="mb-4 text-gray-200 bg-green-900/40 border-green-500/30"
                >
                  <Star size={16} className="mr-2" /> Mentor
                </Badge>
                <div className="flex items-center justify-center gap-2 text-gray-300 mb-6">
                  <Award size={16} />
                  <span className="font-medium">
                    {(user as any)?.experience || "0"}+ Years Experience
                  </span>
                </div>
          ...
          <div className="xl:col-span-2 space-y-6">
              ...
              <div className="mt-8">
                ...
                <div className="flex flex-wrap gap-3">
                  {selectedTags.map((tag, index) => (
                    <span
                      key={index}
                      className="bg-green-900/40 border border-green-500/30 px-4 py-2 rounded-full text-sm font-medium text-green-300 flex items-center"
                    >
                      {tag}
                      <button
            ...
            <SpokelyCard className="p-8 bg-white/10 backdrop-blur-lg border border-white/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {[
                  {
                    val: (user as any)?.sessionsDone || 0,
                    label: "Total Sessions",
                    color: "green",
                  },
                  { val: 4.9, label: "Avg Rating", color: "blue" },
                  { val: "89%", label: "Goal Achieved", color: "purple" },
                ].map((stat, i) => (
                  <div
                    key={i}
          ...
          <div className="xl:col-span-1 space-y-6">
            <SpokelyCard className="p-8 bg-white/10 backdrop-blur-lg border border-white/20">
              ...
              <button
                className="w-full mb-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-4 rounded-xl font-medium flex items-center justify-center"
                onClick={() => setShowEditModal(true)}
              >
                <Edit size={16} className="mr-2" /> Edit Mentor Details
              </button>
              <button
                className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 rounded-xl font-medium flex items-center justify-center"
