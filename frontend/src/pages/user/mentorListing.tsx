    ...
    return (
      <SpokelyCard className="bg-gray-900 border border-gray-700 text-gray-200 hover:border-emerald-400">
        <div className="flex items-center mb-4">
          <div className="w-12 h-12 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex items-center justify-center text-black font-bold text-sm mr-3">
            {mentor?.name?.slice(0, 2).toUpperCase() || "??"}
          </div>
          <div>
            <h3 className="font-semibold">{mentor?.name}</h3>
            <Badge variant="peer" size="sm">{sub.status}</Badge>
          </div>
        </div>
        ...
        <Button variant="primary" onClick={() => navigate(`/user/mentor-profile/${mentor._id}`)}>View Details</Button>
      </SpokelyCard>
    );
  };
